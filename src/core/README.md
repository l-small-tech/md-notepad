...
| `smooth-scroll.ts` | reference | the physics smooth scrolling shares: critically damped spring (velocity carries across retargets), wheel-delta units, the wheel-vs-touchpad classifier, and the notch-unit tracker that makes one notch scroll `NOTCH_LINES` lines on every platform (DOM scrollers in ui/, the terminal viewport in renderer/) |
