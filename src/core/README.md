...
| `prompt-status.ts` | feature | `*.prompts.md` notes as prompts (`isPromptsPath`): the `prompts/STATUSES.md` table agents report progress in (`parseStatuses` — generous; `serializeStatuses` — byte-identical to the bundled `status.py`; `upsertStatus`), prompt ids (`note.md#heading-slug`, GitHub-style slugs with `-1` repeats), `promptSections` / `sectionAtLine`, the clipboard text with its `Prompt-id:` line (`promptText`), `groupByStatus` for the panel |
| `workspace-modules.ts` | feature | Initialize Workspace: a module = directive + seed files; `composeAgentsFile` writes the chosen ones between `<!-- module:id -->` markers (refresh in place, remove unticked, leave unknown ids and all outside text alone), `installedModuleIds` reads them back, `userModuleFrom` turns a user `.md` into a module, `planWorkspaceInit` → the writes (seeds create-only, `status.py` refreshed, stubs/`.gitignore` appended to). Texts, including the Python script, live in `workspace-module-texts.ts`; a test runs the real script against the TS parser |
