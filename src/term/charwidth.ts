/**
 * Display-width classification for codepoints (wcwidth-style).
 *
 * A cell in the grid is 1 column wide; wide characters (CJK, most emoji)
 * occupy 2 cells; combining marks and other zero-width characters occupy 0
 * and attach to the previous cell.
 *
 * The range tables are generated from Unicode 16.0 data (UnicodeData.txt
 * Mn/Me/Cf for zero width, EastAsianWidth.txt W/F plus emoji-data.txt
 * Emoji_Presentation for wide) by `scripts/gen-width-tables.mjs` and checked
 * in, per the plan. Regenerate on a Unicode version bump; do not hand-edit
 * the tables.
 */

/** [start, end] inclusive codepoint ranges, sorted ascending. */
...
/**
 * Columns the codepoint occupies: 0 (combining/zero-width), 1, or 2 (wide).
 *
 * Control characters never reach the grid as printables (the parser routes
 * them to `execute`), so their width here (0) is only a fuzz-safety fallback.
 */
export function charWidth(cp: number): 0 | 1 | 2 {
  if (cp >= 0x20 && cp < 0x7f) return 1; // ASCII fast path
...
/** Zero Width Joiner — glues emoji sequences; the sequence stays in one cell pair. */
