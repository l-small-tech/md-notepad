...
def nebula_sky(rng: np.random.Generator, top: str, bottom: str, nebulae: list[str]) -> np.ndarray:
    ...
    for color in nebulae:
        ...
        sky += a[:, :, None] * hex_rgb(color)[None, None, :] * 0.34
    return sky
...
def bokeh_stars(rng: np.random.Generator) -> tuple[np.ndarray, np.ndarray]:
    ...
    for _ in range(52):
        ...
        rc = r * 0.45
        d.ellipse([x - rc, y - rc, x + rc, y + rc], fill=int(220 * b))
...
def acrylic(
    ...
    scene = nebula_sky(rng, sky_top, sky_bottom, nebulae)
    ...
    out += dust[:, :, None] * hex_rgb(star_tint)[None, None, :] * 0.5
    local = np.clip(out / np.maximum(out.mean(axis=2, keepdims=True), 1e-4), 0.6, 1.6)
    out += big[:, :, None] * hex_rgb(star_tint)[None, None, :] * local * 0.5
