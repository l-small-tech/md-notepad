...
Automated verification is green: `pnpm run check`, **1100 tests** across 77 files (35 new: `skeleton`, `contour`, `trace`, `scanfill` suites), `pnpm run build` (whiteboard still its own lazy chunk, 61.2 → 76.0 KB), `cargo check`.
...
- **Nothing is dropped at trace time.** The plan deferred despeckling here, but the phase-5 conclusion holds at the vector level too: a residue speck and an i-dot both trace to a dot; there is no "no length and no continuation" case that isn't also a legitimate dot. What phase 6 actually delivers on that promise is *tractability*: every speck is now its own one-tap-erasable element in its inherited colour.
