...
- **Text is now a click-DRAG box (the MS Paint gesture).** Drag the text tool out and the dashed rect you drew becomes the box: its top edge is the first line, its width is what the type wraps to; a plain click (under 16 screen px of drag) still opens a box that auto-sizes to what is typed, which is right for a one-word label. The press cannot know which gesture it is, so it only starts a drag and the RELEASE decides. **SVG does not wrap**, so the box's wrapping is baked into `<tspan>` lines on commit by the new pure `core/whiteboard/text-wrap.ts` — fed a canvas `measureText` (injected, so the module stays DOM-free) at the *scene* font size, so the wrap is a property of the document rather than of the current zoom, and the file breaks exactly where the textarea showed it breaking. `TextElement` gained `boxWidth` (`wb:box-width`, omitted when null → older files and auto-sized text round-trip untouched); it records only the box the lines came from, so reopening rewraps to the same width, and a resize scales it by the *horizontal* factor rather than the geometric mean, because a width is a width.
...
Automated verification after this round: `pnpm run check` green, **993 tests** across 67 files (12 new: `text-wrap.test.ts`, box round-trip, box-width scaling, the nib/type proportion), `pnpm run build` green with the whiteboard still its own lazy chunk (32.7 KB), `cargo check` green.
