...
Output long edge = the quality preset (Fast 1200 / **Balanced 1800** / Detailed 3600 px), clamped to the source's resolution. Cost at 1800 px ≈ 3.2 M samples, roughly 0.2–0.5 s on a mid tablet, chunked by row-bands with yielding.
...
Attempted fix, **built and reverted**: paint ink by coverage instead of as a 1-bit stamp — new pure `coverage.ts`, stored on `CleanResult`, consumed by `composeCleaned` as alpha on a transparent sheet and as a blend toward white on an opaque one. The information was already measured and then discarded: extraction decides *whether* a pixel is ink, and the normalized image still knows *how much*. Three details make it correct rather than merely soft: coverage is `255 − min(R,G,B)` (distance from board white, not darkness — a yellow marker found by the chroma gate would otherwise render nearly transparent); each component is normalized against its OWN core, scaled by 0.9 so the plateau saturates and only the rim tapers (coverage answers *where the stroke is*, not how hard it was pressed — the flat per-component colour already carries the marker's identity); and the taper extends one pixel PAST the mask, or it would still land on a hard step at the threshold boundary. Presentation only — mask, components and colours were untouched. It rendered smooth, continuous strokes and UAT rejected it outright: *"looks much worse than before… we're looking for clean lines that can be easily vectorized."*

**That verdict settles what the phase-5 raster is FOR, and it is the durable lesson of the round.** The cleaned image is not a deliverable to be made pretty — it is a *view of the tracer's input*, and the tracer's input is the binary mask. Soft edges make the preview flatter to look at while telling the user nothing about the thing they are actually judging: whether the mask holds clean, continuous, traceable lines. Hard pixels are the honest rendering, aliasing included; a jagged edge on screen is a jagged edge in the mask, and that is information. So: **the phase-5 raster stays 1-bit, and any future complaint about how it LOOKS should be answered in phase 6 with real stroke widths, not by softening the preview.** Faintness at default zoom is a display-scale artifact and is not worth trading legibility of the mask for.

Sixth round — **Detailed raised 2400 → 3600 px** ("could we increase the resolution a little? It's cutting off just a little too much detail"). This is the one lever that genuinely adds information rather than redistributing it, and it addresses the one real defect the fifth round's stage dumps did find: the faint arrow shaft thins to a **1 px hairline** at 2400 and fragments, which is the worst possible input to a skeletonizer. 1.5× the sampling puts that stroke at ~2 px, above the width where the binarizer's decision is all-or-nothing per pixel. Cost is 2.25× the pixels through every stage — which is the trade the preset name promises, and the reason it is a preset rather than the default. The ladder is deliberately no longer evenly spaced: Fast and Balanced are the everyday rungs, Detailed is now "resolve everything the camera saw".

Note this does **not** contradict the fifth round's finding that a higher preset can *look* worse — both are true and they are about different things. Fit-to-pane means more resolution always renders finer on screen; it also means more of the mask survives for the tracer. The first is a preview artifact, the second is the deliverable, and phase 5's job is the second. Android's `MAX_CAPTURE_EDGE` went 2600 → 3800 to match, with the margin deliberate: the board rarely fills the frame, so a cap *at* 3600 would silently clamp Detailed on any shot not framed edge-to-edge.

Also confirmed as a non-issue by the same round: colour fidelity does not matter here (the user's words), because vectorized strokes get their colour from the component vote regardless of what the preview painted.
...
New pure modules in `src/core/whiteboard/scan/`, all colocated-tested: `thin.ts` (Zhang–Suen with an active-frontier queue — O(ink pixels), risk 8's mitigation built in), `skeleton.ts` (pixel graph → polylines: junction clustering, spur pruning to fixpoint, angle-paired continuation), `contour.ts` (marching squares, hole-aware), `trace.ts` (the resumable job, stroke/blob classification, width sampling, `buildScanElements`/`fitScanElements` + the size guard). `smoothing.ts` gained `simplifyIndices` (RDP that reports kept indices, so per-vertex widths survive simplification in lockstep).

Decisions taken during Phase 6 that the spec above did not pin down:

- **`wb:tool="scanfill"` is a first-class StrokeElement, painted with `fill` + `fill-rule="evenodd"`, `stroke="none"`.** The colour stays in the model's `stroke` field (its identity); serialization translates. It hit-tests anywhere inside its filled area (holes excepted — new `pointInPolygonsEvenOdd` in geometry.ts), erases, selects and transforms like any stroke. It takes NO palette-slot class: the palette block's `.wb-cN:not(text)` rule sets `stroke` and would outline every glyph-sized blob; blob theming is a possible later addition and the literal hex renders correctly everywhere.
- **Sauvola gained an absolute-darkness floor** (`L < 0.35` is ink regardless of local stats). Sauvola's low-variance veto — the thing that keeps blank board blank — also fires inside any solid filled region larger than its window, hollowing filled shapes into rings (found by the phase-6 blob golden, invisible in phase 5 because no test board had a filled region). Absolute darkness needs no local evidence: a normalized blank board sits near 255. Sauvola may veto contrast decisions, never darkness.
- **Skeleton adjacency uses the redundant-diagonal rule** (a diagonal neighbour doesn't count when an orthogonal bridge pixel connects the same two): without it every thinned staircase corner manufactures a fake junction and a closed loop shatters into stubs.
- **Junction continuation has a 75° bend limit.** Pair the straightest ends while under it, stop when over: an X merges into two full strokes, a T keeps its stem separate from its crossbar. Deliberately permissive — a fused sharp corner still reads as one pen movement, while a refused genuine crossing shatters into stubs, the worse error.
- **A shallow crossing that SHARES a pixel run may fragment the crossing stroke** into 2–3 pieces plus overlapping stubs (the crossed-out golden pins the contract: the *word*-stroke survives whole, all ink present, bounded fragment count). A shared run is genuinely ambiguous at skeleton level; the pieces overlay the crossing and read correctly.
- **A nib-sized dab (bbox ≤ 1.5·w each way) short-circuits to a DOT** — a single-point stroke at its centre, `2·dtMax` wide. Neither a centerline nor a contour says anything a dot doesn't, and it is what keeps i-dots dots.
- **The despeckle is WIDTH-relative and only ever a conjunction** (revised after the first desktop UAT round — the original "nothing dropped" stance drowned a real board in 2 px residue dots). The vector level has the one discriminator the raster never had: measured ink width in units of the page's own pen width `w`. Ink under `0.5·w` wide AND under `3·w` long is residue and is dropped; thin-but-LONG ink (a fading hairline stroke, a tapering tail) always survives, and nib-width marks (i-dots, commas) are never touched. On the real UAT board this removed all 53 residue dots and every edge wisp while keeping the faintly-drawn circle and the arrow's hairline half.
- **The skeleton graph is width-aware too** (same UAT round). Binarization leaves hairline wisps along rough stroke edges; each wisp fakes a junction that fragments the long stroke it hangs off (the box sides traced as 24 pieces). Dangling residue-thin edges are pruned up to `3·w` (past the spur limit, never past `3·w` — fading tails are thin and long), and solid edges pair with each other in junction continuation before residue edges may pair at all, so a wisp never steals the continuation. Box: 24 paths → 8; total paths on the UAT board: 189 → 49.
- **Every emitted stroke renders at its own PATH median width**, not a component-wide median (same UAT round: the arrow's fat half and hairline half share one component, and the shared median painted the whole arrow 2 px thin while painting edge wisps 6 px fat — the "inconsistently thick" report).
- **Width floor + nick bridging** (UAT round 2 — the faintly-drawn circle rendered near-invisible and broke into arcs). Kept thin-but-long ink renders at no less than `0.5·w` (the despeckle threshold — the two rules compose). And pixel-scale nicks where binarization briefly lost a faint stroke are healed after tracing: endpoint pairs within `2·w`, ACROSS components only (same-component ends are junctions that continuation deliberately refused to fuse), both end tangents collinear across the gap within 50° — so letter spacing never ligatures. Verified against the real board: circle continuous, "Other Place" untouched; the masks overlay proved binarization was NOT the bottleneck (the ring was continuous but for 1–3 px nicks), so no raster thresholds were touched.
- **Covered residue is suppressed** (UAT round 3 — the width floor made the parallel edge-doubling tracks along the box visible again, re-raising the many-small-lines report). A thin path (≤ `0.75·w`) whose vertices sit ≥80% inside the painted band of a strictly wider stroke carries no visible ink of its own: the wider stroke already paints every pixel it would. Dropped after bridging. Thin ink in open space (the faint circle) is covered by nothing and always survives; nib-width retraces are never suppressed. Real board: 47 → 42 strokes, box edges read as single clean lines AND the circle stays closed.
- **The tracer re-runs the EDT on the KEPT mask.** `extraction.distance` measured the weak mask, where rejected neighbours inflate widths; sampled stroke widths must come from surviving ink only.
- **The review's vector preview draws the real elements** (`Path2D` accepts each element's own `d`), themed ink resolved through the live app palette — the preview is the geometry that will land in the file, in the colours it will wear on this board. The view button cycles strokes → cleaned → photo.
- **Colour remap chips apply to every view and both modes.** One chip per detected colour with its mark count; remapping merges into the target's canonical hex (a remapped colour has no measured value — the user's choice IS the answer). The remap travels with the insert payload.
...
**Phase 7 — OCR (S6). ✅ BUILT** (worktree `worktrees/whiteboard-work` on `feat/whiteboard-work`; desktop UAT pending, tablet QA pending). `text-layout.ts` grouping, the recognizer port, Android ML Kit chain (Digital Ink primary, Text Recognition fallback), **a real Windows engine instead of the planned desktop no-op** (`Windows.Media.Ocr`), `<desc>` + hidden `<text>` + metadata JSON, "Copy text" in review, async insert-first-patch-later flow. macOS/Linux record `"status":"unavailable"`.
...
Automated verification is green: `pnpm run check`, **1135 tests** across 79 files (35 new: `text-layout`, `ocr`, history-replace suites), `pnpm run build` (whiteboard still its own lazy chunk, 76.0 → 87.3 KB), `cargo fmt --check`, `cargo clippy --all-targets -D warnings`, `cargo test` (49), `cargo check` and `cargo check --target aarch64-linux-android`.

Decisions taken during Phase 7 that the spec above did not pin down:

- **Windows gets a real engine, not the no-op** (user requirement: the feature may run on a Windows 11 tablet with a camera). `Windows.Media.Ocr` is on-device, offline, ships with Windows 10/11, and costs zero new crates — the `windows` crate was already in the tree via tauri/wry; the direct dependency only enables the WinRT namespaces (`commands/ocr.rs`, cfg-gated like the Android module). It is a printed-text model — good on block capitals, weak on cursive — and the metadata names the engine so a consumer can weigh that. The cleaned raster goes over IPC as the black-on-white PNG; boxes come back in its pixel space, rescaled in Rust when the engine's `MaxImageDimension` (2600 on current builds) forces a downscale of a Detailed scan. No confidence values — recorded as `null`, never invented. macOS/Linux keep the honest `unavailable` (Vision/objc would be new deps against the freeze; tesseract.js was already rejected in the S6 table).
- **The OCR representation is RawElements + metadata, zero serializer changes.** The `<desc>` and `<g wb:ocr="text" opacity="0">` group are built as one-line raw XML (self-escaped, newline-encoded as `&#10;`) and ride the verbatim re-emission path phase 1 built — the roundtrip golden already pinned the shape. `applyScanOcr` (pure, in `scan/ocr.ts`) removes the old pair, prepends the new, and writes `ocr[layerId]` into the doc metadata with deterministic key order; the fixed-point goldens cover ok / empty / unavailable / re-run.
- **Recognition starts when the trace lands, not at insert.** It runs while the user reviews, so "Copy text" is usually ready by the time they look for it; the outcome PROMISE rides the `ScanStrokesResult` payload, so an insert that beats the engine still gets its text — the adapter patches the layer when the promise settles. The panel's `generation` token discards a stale result after Back/retake/close.
- **The async patch is undo-invisible via `History.replace`** (new, tested): it swaps the current snapshot instead of pushing, so undo steps OVER the annotation and redo after an undo lands on the annotated state. The patch re-locates the scan layer by id against the CURRENT scene and drops the result if the layer is gone (deleted, undone, file reloaded) — `applyScanOcr` returning null is the contract. Known small race, accepted: a select-drag in progress when the patch lands commits from its drag-start snapshot and can clobber the annotation; only the annotation is lost, and a re-scan regains it.
- **The layout gate fails toward "diagram".** `text-layout.ts` groups by y-band overlap, splits bands at x-gaps, absorbs i-dot satellites, and refuses lines with inconsistent heights or box/arrow proportions — an unsubmitted word costs a line of `<desc>`, a submitted arrow costs the engine hallucinating text onto geometry. Layout indices ARE element indices (same enumeration order as `buildScanElements`), which is what lets metadata lines carry the `s1..sN` ids assigned at insert; element count is ε-independent, so the size guard can't skew the mapping.
- **Ink payloads cross the Android bridge as one JSON string** parsed with org.json — the existing `@InvokeArg` fields are all flat, and a JSON pass-through keeps the payload shape owned by exactly two files (the TS builder and the Kotlin parser). Digital Ink gets one ink PER LINE in reading order with a writing-area hint (risk 7's mitigation), synthetic monotonic timestamps, and the language model downloads on first use — recognition being async end-to-end means a slow first scan just patches late. The raster fallback fires when the ink model is missing, fails, or reads nothing at all.
- **Raster engines read the whole board, layout or no layout** — engine line boxes attach items by centre containment, so text the classifier missed still lands (with empty `strokes` lists), while ink engines are only ever fed layout-approved lines.

First desktop UAT round (the real "Box 1 → Other Place" board): no `<desc>` appeared. The metadata told the story precisely — `{status:"ok", engine:"windows-ocr", lines:[]}` — the chain worked end to end and the ENGINE read nothing. Diagnosed offline with WinRT probes (PowerShell 5.1 projection) and the photo harness; three findings, one fix, one removal:

- **`Windows.Media.Ocr` cannot read handwriting, full stop — and it is not our input's fault.** Zero lines from the raw photo at five scales, zero from the cleaned black-on-white raster, zero from a System.Drawing re-render of the traced strokes at marker weight (a rendering trivially legible to a human) — while the same engine reads a typed-Arial control perfectly, first try. The engine is healthy; it is a printed-text model with no handwriting capability. The Windows raster path STAYS (it costs one IPC call, records an honest empty, and genuinely reads printed content — labels, projected slides, neat print), but on real handwriting the honest Windows answer is `lines: []`.
- **`InkAnalyzer` (the OS handwriting engine) was built, probed, and REMOVED.** It activates fine from an unpackaged desktop process and reads synthetic pen strokes ("HI") — but on the tracer's real output it returned confident junk at every coordinate scale (1×–0.1×), isolated per cluster or whole-board, auto-classified or forced-writing: "Box 1" → `x-` `.` `1`, "Other Place" → `5 7` `lace` `01`. The cause is structural, not tunable: centerline skeletons carry none of the stroke dynamics (direction, order, speed, letter segmentation) the recognizer models — a skeleton "B" with junction-fused bowls is an impossible pen gesture. Junk in a `<desc>` is worse than an honest empty (the glare principle again), so the path was deleted the same day it was written. Bring NEW evidence (e.g. a raster-free engine that demonstrably reads traced strokes) before reintroducing.
- **The layout gate met its first real board and lost "Box 1" to two structural effects.** (a) Two text scales on one page: the page-median height made the ~300 px "B"/"1" read as diagram-sized — geometrically indistinguishable from the circle right next to them, so this stays a KNOWN LIMITATION rather than a knob. (b) Broken arrow-shaft fragments (13–20 px tall, 100–280 px wide) shared a y-band with the "ox" letters and dragged the band's height consistency past the gate — fixed with a **sliver rule** (short + aspect > 6 → diagram, out of the bands), unit-tested and verified against the board via the harness. Arrowheads remain letter-shaped and letter-sized; no geometric rule separates them, which is the ink engines' problem to survive (Android QA will judge ML Kit here).
- **The photo harness grew phase-7 output**: per-item layout classification, line dumps, and `ocr-payload.json` — the exact ink payload the recognizers get, ready to feed an engine probe offline. That loop (harness → probe → verdict in minutes, no app in the way) is what made this round's conclusions cheap.

Net Windows position after the round, honestly stated: scans OCR **printed** content on-device; real handwriting records `lines: []` until either the opt-in cloud engine (v1.5) or a future on-device model worth its dependencies. Handwriting recognition on-device rides on Android's ML Kit Digital Ink — which, unlike InkAnalyzer, is trained with synthetic-stroke augmentation and is fed line-level ink — pending tablet QA.

**Second UAT round — the review screen itself** (branch `feat/wb-scan-review`). Three changes, all in `whiteboard-scan.ts` plus one new `src/ui/scan-debug.ts`:

- **The "Ready to insert…" summary card is gone.** It was true and useless: dimensions, stroke and colour counts, and a sentence about layers, permanently occupying a fifth of the pane the screen exists to show. The heading now appears ONLY when something is wrong (no ink found, glare over the hint threshold, size guard triggered, absurdly dense board); the counts moved into the insert buttons' tooltips, available on demand.
- **The preview zooms.** Pinch, wheel and double-tap-to-fit on the same `core/diagram-zoom.ts` math the board itself uses, applied as one composited transform on the canvas (origin 0 0), so no re-rasterization and no new module. Past 1:1 the canvas switches to `image-rendering: pixelated` — at that zoom the question is "what exactly did the tracer draw here", which is a pixel question. A fit happens once per new image; recolours and view toggles keep the user's zoom, because they are comparing the same detail across views.
- **"Debug insert"** performs the ordinary insert (same code path — a debug button that exercises different code proves nothing) and additionally writes `scan-debug-<timestamp>/` beside the whiteboard containing `1-source.jpg` (the camera's own bytes, pre-decode), `2-straightened.jpg`, `3-cleaned.png` and `4-traced.svg` (the same `d` strings that land in the board, plus recognized text in a `<desc>`). Which stage lost the ink is then visible by opening four files in order. The write is injected from `EditorHost` through the storage provider, so it works on SAF/synced workspaces too, and is deliberately awaited AFTER the insert — the user should not wait on a disk round-trip to see their board.
