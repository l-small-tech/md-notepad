...
- **The i-dot rule was a pass-through, not a filter.** It spared any speckle with kept ink within 2·w — and near handwriting there is kept ink within 2·w of *everything*, so nothing was ever despeckled. Proximity is still necessary but no longer sufficient: a spared speckle must also look like the marker made it, either a **dab** (`dtMax ≥ 0.3·w` — a pen cannot draw thinner than its own tip, so a real i-dot, colon or accent clears this and dust does not) or a **fragment of a line** (spanning ≥ `w` on one axis — what keeps a dashed arrow shaft and a fading box edge). Grit is both thin and short and now dies. On the real board: 123 → 32 components, 91 speckles removed, every reported dot gone, and the faint arrow fragments and box edge still present. The golden's i-dot grew from 2×2 to 4×4 — physically it always should have been, since the strokes around it are 5 px — and gained two new cases: grit 6 px from a stroke (must die) and a 12×1 faint dash (must live).
...
Third UAT round — the dots were confirmed gone (and confirmed to be real residue marks on the physical board, not pipeline noise), but the correction had overshot: **the arrow read faint and the circle lost chunks.** The mechanism was the FRAGMENT test's floor. A light stroke does not fade into neat span-`w` pieces; it fades into pieces *shorter* than `w`, and those failed both shape tests. Fix: **a `rescued` component is exempt from the size filter entirely.** The continuity rescue has already established weak-only + page-ink thickness + continuity with kept ink — strictly stronger evidence than either shape test, and indifferent to size. The two populations stay separable because they differ in the one property that matters: dark residue carries strong pixels, so hysteresis admits it directly and the rescue never considers it; faint ink is weak-only and the rescue is the only thing that can speak for it. The golden gained a fading tail of 4-px weak-only dashes off a stroke end, asserting **every** dash survives — a gappy circle only shows up further along the fade, so checking the first piece would not have caught this.

Known limit recorded rather than papered over: `w` is one number for the whole page, so a fine-tip dot on a board dominated by fat markers can still read as grit. Mixed nib widths are a phase-6 concern. Also still open from round 1: the cleaned PNG bakes the ink colours resolved at insert time, so a board scanned in dark theme keeps dark-theme ink when viewed in light theme (`whiteboard-2.svg` shows this — near-white specks, grey ink). Phase 6's vector strokes theme live and retire it.

Automated verification after this round: `pnpm run check` green, **1073 tests** across 73 files, `pnpm run build` green, `cargo check` green.
