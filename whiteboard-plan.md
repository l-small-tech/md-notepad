...
Automated verification is green: `pnpm run check`, 978 tests (57 new: `select.test.ts`, `input.test.ts`, text-tool and store additions), `pnpm run build` (whiteboard still its own lazy chunk, 21.6 → 29.8 KB), `cargo check`.
