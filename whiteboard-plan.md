...
Still open, unchanged: `w` is one number for the whole page, so mixed nib widths are a phase-6 concern; and the cleaned PNG bakes the ink colours resolved at insert time, so a board scanned in dark theme keeps dark-theme ink when viewed in light theme (`whiteboard-2.svg` shows this). Phase 6's vector strokes theme live and retire it. **Unresolved and now the top phase-6 input: "Detail mode cuts out too much."** Ink loss on faint strokes was reported independently of the speckle work and was never reproduced locally — diagnosis has been running on the *processed output*, which cannot show what the binarizer discarded. This needs the source photograph.
