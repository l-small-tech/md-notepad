...
- **Two palettes: themed and static.** The ribbon gained an Auto/Fixed toggle (`paletteKind` in the whiteboard store) switching the swatch row between the 8 themable slots and `STATIC_PALETTE` — eight standard *named* SVG colours (`black`, `red`, … `purple`). Named colours can never equal a `PALETTE` hex, so the derived-class rule ignores them and a static stroke is literal by construction — zero format machinery. The rows share hue order and switching carries the selection across by index.
